...
func NewBidUseCase(bidRepository bid_entity.BidEntityRepository) BidUseCaseInterface {
	getMaxPatchSizeInterval := getMaxPatchSizeInterval()
	maxBatchSize := getMaxBatchSize()

	bidUseCase := &BidUseCase{
		BidRepository:       bidRepository,
		maxBatchSize:        maxBatchSize,
		batchInsertInterval: getMaxPatchSizeInterval,
		timer:               time.NewTimer(getMaxPatchSizeInterval),
...
func (bu *BidUseCase) triggerCreateRoutine(ctx context.Context) {
	go func() {
		...
		for {
			select {
			// ok - variável que o gol cria para dizer se está tudo ok dentro do channel
...
func getMaxPatchSizeInterval() time.Duration {
